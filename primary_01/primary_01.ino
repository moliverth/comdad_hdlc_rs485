...
#define DEBUG_LEVEL 1 // 1 show full frames, 2 show full frames + received byte
...
uint8_t processFrame(bool reset_recv_seq_num = true){
  ...
  if (not getBit(frame.buffer[2], 7)){ // Information frame process
    ...
    sendSuperFrame(linked_device, (crc_ok ? 0b00 : 0b01), not crc_ok); // {ADDRESS, SUPERVISORY CODE - 0b00(RR) or 0b01(REJ), POOL or FINAL} 
    resetFrameBuffer();

    if ((String)payload_ == "route Heyyy! to 0x03" and crc_ok){
      String message = "Heyyy!";
      linked_device = 0x03;
      Serial.print((String)"\n>> send:'"+message+"' to 0x03");
      addToPayloadBuffer(message);
      delay(100);
      return 8; // Response READY 
    }

    if ((String)payload_ == "route hELLooo! to 0x02" and crc_ok){
      String message = "hELLooo!";
      linked_device = 0x02;
      Serial.print((String)"\n>> send:'"+message+"' to 0x02");
      addToPayloadBuffer(message);
      delay(100);
      return 8; // Response READY 
    }

    if ((String)payload_ == "route Byyye! to 0x03" and crc_ok){
      String message = "Byyye!";
      linked_device = 0x03;
      Serial.print((String)"\n>> send:'"+message+"' to 0x03");
      addToPayloadBuffer(message);
      delay(100);
    ...
    if ((String)payload_ == "route See u! to 0x02" and crc_ok){
      String message = "See u!";
      linked_device = 0x02;
      Serial.print((String)"\n>> send:'"+message+"' to 0x02");
      addToPayloadBuffer(message);
      delay(100);
...
void loop(){
  ...
  if (message.length() > 0){
    if (message[message.length()-1] == 0x0d) message.remove(message.length()-1); // remove CR
    if (not message.length()) return; 
    Serial.print((String)"\n>> send:'"+message+"' to");
    while (not Serial.available()) delay(10);
    uint8_t forward_address = (uint8_t)Serial.parseInt();
    Serial.print(forward_address > 0xf ? " 0x" : " 0x0"); Serial.print(forward_address, HEX);

    addToPayloadBuffer(message);
    sendMessage(forward_address);
  }
