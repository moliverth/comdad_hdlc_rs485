...
uint8_t processFrame(bool reset_recv_seq_num = true){
  ...
  if (not getBit(frame.buffer[2], 7)){ // Information frame process
    ...
    sendSuperFrame(linked_device, (crc_ok ? 0b00 : 0b01), not((String)payload_ == "ok?" or (String)payload_ == "send your route request" or (String)payload_ == "hELLooo!" )); 
    resetFrameBuffer();
    ...
    if ((String)payload_ == "send your route request" and crc_ok){
      String message = "route Heyyy! to 0x03";
      Serial.print((String)"\n>> send:'"+message+"' to Primary");
      addToPayloadBuffer(message);
      return 8; // Response READY 
    }

    if ((String)payload_ == "hELLooo!" and crc_ok){
      String message = "route Byyye! to 0x03";
      Serial.print((String)"\n>> send:'"+message+"' to Primary");
      addToPayloadBuffer(message);
      return 8; // Response READY 
    }
